...
export async function importServersYml(
	...
	if (!(await file.exists())) {
		throw new Error(`no servers.yml at ${serversYmlPath()}`);
	}
	...
	if (!document || typeof document !== "object") {
		throw new Error("servers.yml did not parse as a mapping");
	}
	...
	for (const [rawKey, rawDefinition] of Object.entries(definitions)) {
		...
		if (!name) {
			warnings.push(`servers.${rawKey}: no instance by that name in cluster.json — skipped`);
		...
		if (hostName && hostName !== expectedHost) {
			warnings.push(
				`server-info.${key}.host-name is "${hostName}" but ${name} runs on "${expectedHost}" — the generated file will say "${expectedHost}"`,
			);
		}
		...
		if (typeof definition.page !== "number" || typeof definition.slot !== "number") {
			warnings.push(`servers.${key}: no explicit page/slot — pinned to page ${entry.page} slot ${entry.slot}`);
		}
		...
		if (definition.template && typeof definition.template === "object") {
			...
			warnings.push(
				`servers.${key}: carries a per-server template block — kept as-is, the editor will not touch it (it replaces the global template wholesale)`,
			);
		}
	...
	for (const [rawKey, rawInfo] of Object.entries(serverInfo)) {
		...
		if (!name) {
			warnings.push(`server-info.${rawKey}: no instance by that name in cluster.json — skipped`);
...
export async function applySelector(
	...
	if (errors.length > 0) {
		...
		checking.say("error", `${errors.length} error(s): ${errors[0]?.path} — ${errors[0]?.message}`);

		throw new Error(`selector configuration has ${errors.length} error(s); fix them before applying`);
	}

	checking.complete(issues.length ? `${issues.length} warning(s)` : "no issues");
	...
	if (reload.ok) {
		...
		reloading.complete("proxy reloaded — backends pick it up on their next heartbeat");
	...
	reloading.say("info", "the proxy dropped the connection while reloading — waiting for it to come back");
	...
	reloading.say("error", reload.error ?? "the proxy did not answer");
