 ...
 *
 * The file and schema are PCF 1.2+'s (`version = 2.0`); the 1.1.x line read
 * `config/pcf-common.toml` instead, but 1.2+ covers every MC version luna can
 * provision, so a resolve never lands the old line. Missing keys are filled in
 * by the mod's own spec correction, which keeps the values already present.
 */
const FORGE_PROXY: ForwardingMod = {
	provider: "modrinth",
	slug: "proxy-compatible-forge",
	configFile: "config/proxy-compatible-forge.toml",
